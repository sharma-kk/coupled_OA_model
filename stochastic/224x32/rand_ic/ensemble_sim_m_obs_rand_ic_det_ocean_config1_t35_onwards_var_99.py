...
ua_.dat.data[:] = ua_array + 0.2*np.random.normal()*ua_array
# Ta_.dat.data[:] = Ta_array + 0.0005*np.random.normal()*Ta_array
...
if ensemble_comm.rank%pvd_print == 0: # printing results corresponding to every pvd_print particle
    ...
    outfile = VTKFile('../results/OU_sim_var_99_mesh_32_particle_'+str(ensemble_comm.rank + 1)+'_det_ocean_config1_t'+str(int(init_time))+'_onwards_rand_ic.pvd', comm = spatial_comm)
...
while (round(t,4)<=t_end):
    ...
    if iter_n%freq1 == 0:
        ...
        data_file = '../data_stoch_more_obs/vel_temp_data_particle_'+str(seed_no)+'_var_99_grid_32_t'+str(int(init_time))+'_onwards_OU_rand_ic_m_obs.npz'
