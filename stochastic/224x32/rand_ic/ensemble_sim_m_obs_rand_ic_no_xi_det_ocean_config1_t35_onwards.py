...
if ensemble_comm.rank%pvd_print == 0: # printing results corresponding to every pvd_print particle
    ...
    outfile = VTKFile('../results/ensem_sim_no_xi_det_ocean_mesh_32_particle_'+str(seed_no)+'_config1_t'+str(int(init_time))+'_onwards_rand_ic.pvd', comm = spatial_comm)
...
while (round(t,4)<=t_end):
    ...
    if iter_n%freq1 == 0:
        ...
        data_file = '../data_stoch_more_obs/vel_temp_data_ensem_sim_no_xi_det_ocean_particle_'+str(seed_no)+'_grid_32_t'+str(int(init_time))+'_onwards_rand_ic_m_obs.npz'
