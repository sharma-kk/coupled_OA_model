...
var = 99 # variance level in percentage
res = 32
Dt = 0.04
init_time = 25.0
...
if ensemble_comm.rank%pvd_print == 0: # printing results corresponding to every pvd_print particle
    ...
    outfile = VTKFile('../results/gauss_sim_var_'+str(var)+'_mesh_32_particle_'+str(ensemble_comm.rank + 1)+'_det_ocean_config1_t'+str(int(init_time))+'_onwards_same_ic.pvd', comm = spatial_comm)
...
n_EOF = 53 # desired no. of EOFs to include in stochastic sim
#######################
# 53 EOFs >>>>>> 99 % variance
# 20 EOFs >>>>>> 90 % variance
# 10 EOFs >>>>>> 70 % variance
...
rand_mat = np.random.normal(size=(n_t_steps+2, n_EOF)) # time-series obtained from Gaussian distribution
...
while (round(t,4)<=t_end):
    ...
    if iter_n%freq1 == 0:
        ...
        # data_file = '../data_from_stochastic_run/vel_temp_data_particle_'+str(seed_no)+'_var_'+str(var)+'_grid_32_t'+str(int(init_time))+'_onwards_gauss_same_ic.npz'
